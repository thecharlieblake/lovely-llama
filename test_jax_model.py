...
from jax_model import Dropout, FFNSwiGLU, RMSNorm, rope
from torch_model import FeedForward as TorchFFNSwiGLU
from torch_model import RMSNorm as TorchRMSNorm
from torch_model import apply_rotary_emb, precompute_freqs_cis
...
def test_dropout():
    p = 0.9
    x = jrandom.normal(X_KEY, (2**16,))
    dropout = Dropout(p)
    y = vmap(dropout, in_axes=(0, 0))(x, jrandom.split(KEY, 2**16))

    num_zero = (y == 0).sum()
    assert num_zero == pytest.approx(2**16 * p, rel=0.01)

    num_non_zero = (y != 0).sum()
    var_non_zero = ((y**2).sum() / num_non_zero) - (y.sum() / num_non_zero) ** 2
    std_non_zero = var_non_zero**0.5

    expected_scale = 1 / (1 - p)
    assert std_non_zero == pytest.approx(expected_scale, rel=0.01)

    assert (
        vmap(partial(dropout, inference=True), in_axes=(0, 0))(
            x, jrandom.split(KEY, 2**16)
        )
        == 0
    ).sum() == 0

    grads = filter_grad(lambda x, key: vmap(dropout, in_axes=(0, 0))(x, key).sum())(
        x, jrandom.split(KEY, 2**16)
    )
    print(len(grads), grads.max())
    assert grads.sum() == num_non_zero * expected_scale
...
def test_ffn_swiglu():
    ...
    loss = lambda model, x: vmap(model)(x).mean()
...
def test_rope():
    ...
    loss = lambda x: rope_full_shape(x).mean()
    g = grad(loss)(x)
